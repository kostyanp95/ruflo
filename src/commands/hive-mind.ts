...
You MUST use ruflo MCP tools for ALL coordination. This is NOT optional.

1. BEFORE starting work:
   - Call mcp__ruflo__hive-mind_status to verify swarm health
   - Call mcp__ruflo__memory_search to check for prior knowledge on this topic

2. DURING work:
   - Create tasks via mcp__ruflo__task_create (NOT just native TaskCreate)
   - Store ALL findings via mcp__ruflo__memory_store with namespace "hive-findings"
   - Store discovered patterns via mcp__ruflo__hooks_intelligence_pattern-store
   - Use mcp__ruflo__hive-mind_broadcast to announce progress

3. AFTER completing work:
   - Mark tasks done via mcp__ruflo__task_complete
   - Store final results via mcp__ruflo__memory_store with namespace "hive-results"
   - Call mcp__ruflo__neural_train to learn from the session
   - Report status via mcp__ruflo__hive-mind_status

You may ALSO use native Claude Code tools (Bash, Read, Grep, Agent) for the actual work,
but coordination and knowledge persistence MUST go through mcp__ruflo__* tools.

🚀 BEGIN HIVE MIND COORDINATION NOW!
Start by calling mcp__ruflo__hive-mind_status, then mcp__ruflo__memory_search for prior knowledge, then proceed with the objective.
`;
}
...
async function spawnClaudeCodeInstance(
  ...
  try {
    ...
    // Check if claude command exists and resolve its path
    let claudeAvailable = false;
    let claudeBin = 'claude';
    const claudeSpawnArgs: string[] = [];
    try {
      if (process.platform === 'win32') {
        // On Windows, .cmd shims can't be spawned with shell:false, and shell:true
        // corrupts large multi-line arguments. Instead, resolve the underlying Node.js
        // entry point and spawn node directly.
        const paths = execSync('where claude', { encoding: 'utf8' }).trim().split(/\r?\n/);
        ...
        if (cmdPath) {
          ...
          try {
            accessSync(cliJs);
            claudeBin = process.execPath; // node.exe
            claudeSpawnArgs.push(cliJs);
          } catch {
            // cli.js not found, fall back
            claudeBin = cmdPath;
          }
        }
      } else {
    ...
    if (claudeAvailable && !dryRun) {
      ...
      // Spawn claude with properly ordered arguments — use resolved path (no shell needed)
      const claudeProcess = childSpawn(claudeBin, [...claudeSpawnArgs, ...claudeArgs], {
        stdio: 'inherit',
        shell: false,
      });

...
// Swarm store paths (mirrors swarm-tools.ts for cross-store registration)
...
/**
 * Register an agent in a swarm's agents[] array.
 * If swarmId is provided, uses that specific swarm.
 * Otherwise, auto-detects the most recent running swarm.
 * Returns info about the registration or null if no swarm found/available.
 */
function registerAgentInSwarm(agentId: string, swarmId?: string): { swarmId: string; agentCount: number } | null {
  const swarmStore = loadSwarmStore();

  let target: SwarmState | undefined;

  if (swarmId) {
    target = swarmStore.swarms[swarmId];
    if (!target) return null;
    if (target.status !== 'running' && target.status !== 'initializing') return null;
  } else {
    // Auto-detect: find the most recent running swarm
  ...
  // Check capacity
  if (target.agents.length >= target.maxAgents) return null;

  // Avoid duplicate registration
  if (!target.agents.includes(agentId)) {
    target.agents.push(agentId);
    target.updatedAt = new Date().toISOString();
    saveSwarmStore(swarmStore);
  }

  return { swarmId: target.swarmId, agentCount: target.agents.length };
}

/**
 * Remove an agent from all swarms that contain it.
 * Returns the list of swarm IDs the agent was removed from.
 */
function unregisterAgentFromSwarms(agentId: string): string[] {
  const swarmStore = loadSwarmStore();
  const removedFrom: string[] = [];

  for (const swarm of Object.values(swarmStore.swarms)) {
    const idx = swarm.agents.indexOf(agentId);
    if (idx !== -1) {
      swarm.agents.splice(idx, 1);
      swarm.updatedAt = new Date().toISOString();
      removedFrom.push(swarm.swarmId);
    }
  }
...
export const agentTools: MCPTool[] = [
  {
    ...
    inputSchema: {
      ...
      properties: {
        ...
        swarmId: { type: 'string', description: 'Swarm ID to register agent in (auto-detects running swarm if omitted)' },
      },
      required: ['agentType'],
    },
    handler: async (input) => {
      ...
      // Register agent in swarm (explicit swarmId or auto-detect running swarm)
      const swarmRegistration = registerAgentInSwarm(agentId, input.swarmId as string | undefined);
      ...
      // Include swarm registration info
      if (swarmRegistration) {
        response.swarmId = swarmRegistration.swarmId;
        response.swarmAgentCount = swarmRegistration.agentCount;
      }
  ...
  },
  {
    ...
    handler: async (input) => {
      ...
      if (store.agents[agentId]) {
        ...
        // Remove agent from all swarms it belongs to
        const removedFromSwarms = unregisterAgentFromSwarms(agentId);

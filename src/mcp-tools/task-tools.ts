...
export const taskTools: MCPTool[] = [
  ...
  {
    name: 'task_assign',
    description: 'Assign a task to one or more agents. Optionally auto-execute via headless Claude Code subprocess.',
    category: 'task',
    inputSchema: {
      type: 'object',
      properties: {
        ...
        autoExecute: { type: 'boolean', description: 'Auto-execute task via headless Claude Code subprocess (default: false)' },
      },
      required: ['taskId'],
    },
    handler: async (input) => {
      ...
      if (!task) {
        return { success: false, taskId, error: 'Task not found', autoExecuted: false };
      }
      ...
      // --- Optional auto-execution via HeadlessWorkerExecutor ---
      const autoExecute = input.autoExecute === true;
      let autoExecuted = false;
      let executionError: string | undefined;
      let executionId: string | undefined;

      if (autoExecute && !input.unassign && task.assignedTo.length > 0) {
        try {
          // Lazy dynamic import to avoid breaking load when dependencies are absent
          const { HeadlessWorkerExecutor } = await import(
          ...
          if (available) {
            // Fire-and-forget: launch execution in background, track PID via pool status
            executionId = `task-exec_${taskId}_${Date.now()}`;

            // Non-blocking launch: use setImmediate + catch to avoid blocking the response
            const executionPromise = executor.execute('deepdive', {
              promptTemplate: `Execute the following task:\n\nTask ID: ${taskId}\nType: ${task.type}\nDescription: ${task.description}\nPriority: ${task.priority}\n\nAnalyze the codebase and perform the described task. Provide a detailed result.`,
              sandbox: 'strict',
              outputFormat: 'text',
            });

            // Attempt to grab PID from pool status after launch starts.
            // The executor tracks spawned child processes in its pool; the most
            // recently started 'deepdive' entry corresponds to our launch.
            try {
              const poolStatus = executor.getPoolStatus();
              const activeEntry = poolStatus.activeWorkers.find(
                (w: { workerType: string }) => w.workerType === 'deepdive'
              );
              if (activeEntry) {
                // executionId from pool can be used for monitoring
                executionId = activeEntry.executionId;
              }
            } catch { /* pool query is best-effort */ }

            // Store execution metadata in task record
            const updatedStore = loadTaskStore();
            const updatedTask = updatedStore.tasks[taskId];
            if (updatedTask) {
              updatedTask.result = {
                ...(updatedTask.result || {}),
                autoExecution: {
              ...
              saveTaskStore(updatedStore);
            }

            // Background completion handler: update task when execution finishes
            setImmediate(() => {
              executionPromise
                .then((execResult) => {
                  const completionStore = loadTaskStore();
                  const completionTask = completionStore.tasks[taskId];
                  if (completionTask) {
                    completionTask.result = {
                      ...(completionTask.result || {}),
                      autoExecution: {
                        executionId,
                        startedAt: (completionTask.result as Record<string, unknown>)?.autoExecution
                          ? ((completionTask.result as Record<string, Record<string, unknown>>).autoExecution.startedAt)
                    ...
                    if (execResult.success) {
                      ...
                      completionTask.completedAt = new Date().toISOString();
                    } else {
                      completionTask.status = 'failed';
                    }
                    saveTaskStore(completionStore);
                  }
                })
                .catch(() => {
                  // Best-effort: silently handle background errors
                  const errorStore = loadTaskStore();
                  const errorTask = errorStore.tasks[taskId];
                  if (errorTask) {
                    errorTask.result = {
                      ...(errorTask.result || {}),
                      autoExecution: {
                        executionId,
                        status: 'failed',
                        error: 'Background execution crashed',
                      },
                    };
                    errorTask.status = 'failed';
                    saveTaskStore(errorStore);
                  }
                });
            });
            ...
            executionError = 'Claude Code CLI not available for headless execution';
          }
        } catch (err) {

        ...
        handler: async (input) => {
            ...
            if (autoExecute && !input.unassign && task.assignedTo.length > 0) {
                try {
                    ...
                    const { HeadlessWorkerExecutor } = await import('../services/headless-worker-executor.js');
                    ...
                    if (available) {
                        ...
                        try {
                            ...
                            const activeEntry = poolStatus.activeWorkers.find((w) => w.workerType === 'deepdive');
                            if (activeEntry) {
                            ...
                            executionPromise
                                ...
                                if (completionTask) {
                                    completionTask.result = {
                                        ...
                                        autoExecution: {
                                            executionId,
                                            startedAt: completionTask.result?.autoExecution
                                                ? (completionTask.result.autoExecution.startedAt)

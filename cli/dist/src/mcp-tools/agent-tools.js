 ...
 */
function registerAgentInSwarm(agentId, swarmId) {
    ...
    if (swarmId) {
        ...
        if (!target)
            return null;
        if (target.status !== 'running' && target.status !== 'initializing')
            return null;
    }
    else {
    ...
    // Check capacity
    if (target.agents.length >= target.maxAgents)
        return null;
 ...
 */
function unregisterAgentFromSwarms(agentId) {
    const swarmStore = loadSwarmStore();
    const removedFrom = [];
...
export const agentTools = [
    {
        ...
        handler: async (input) => {
            ...
            const swarmRegistration = registerAgentInSwarm(agentId, input.swarmId);

...
/**
 * Detect the MCP server name for claude-flow/ruflo from the project's .mcp.json.
 * Checks common names: 'ruflo', 'claude-flow', '@claude-flow/cli'.
 * Falls back to 'claude-flow' if no .mcp.json found.
 */
async function detectMcpServerName() {
    ...
    try {
        if (existsSync(mcpJsonPath)) {
            ...
            // Check for known server names in priority order
            for (const name of ['ruflo', 'claude-flow', '@claude-flow/cli']) {
                if (servers[name])
                    return name;
            }
            // Check if any server uses claude-flow CLI
            for (const [name, server] of Object.entries(servers)) {
                const s = server;
                const args = s.args || [];
                const cmd = s.command || '';
                const allParts = [cmd, ...args].join(' ');
                if (allParts.includes('claude-flow') || allParts.includes('ruflo')) {
                    return name;
                }
            }
        }
 ...
 */
function generateHiveMindPrompt(swarmId, swarmName, objective, workers, workerGroups, flags, mcpServerName = 'claude-flow') {
...
async function spawnClaudeCodeInstance(swarmId, swarmName, objective, workers, flags) {
    ...
    spinner.start();
    try {
        // Detect MCP server name from project's .mcp.json
        const mcpServerName = await detectMcpServerName();
        // Generate comprehensive Hive Mind prompt
        const workerGroups = groupWorkersByType(workers);
        const hiveMindPrompt = generateHiveMindPrompt(swarmId, swarmName, objective, workers, workerGroups, flags, mcpServerName);
